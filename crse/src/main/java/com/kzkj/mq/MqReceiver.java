...
//          String errorMsg = this.validateXml(xml);
    ...
    public void onMessage(Message message, Channel channel) {
        ...
        try
        {
            ...
            //判断发送方id是否存在
            List<Company> companyList= companyService.selectList(null);
            String dxpId = "";
            if (companyList != null)
            {
            ...
            if(!flag)
            {
                log.info("dxpId不存在["+dxpId+"]",customsXml);
                //String errorMsg = "发送方id不存在["+dxpId+"]";
                //sendCEB900Message(errorMsg, xml, customsXml, receiveXml);
                return;
            }
            flag=true;
            ...
            if(!flag)
            {
                log.info("接受方id不对应{}",customsXml);
                String errorMsg = "接受方id不对应["+customs.getTransInfo().getReceiverIds().getReceiverId().get(0)+"]";
                sendCEB900Message(errorMsg, xml, customsXml, receiveXml);
                return;
            }
            //校验报文格式
    ...
    private void sendCEB900Message(String errorMsg, String xml, String customsXml, String receiveXml) {
        ...
        messageReturn.setOriBizMessage("");
